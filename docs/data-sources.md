...
|:---|:---|:---|:---|:---|:---|
| City of Portland School_Boundaries (AGOL), School Attendance Areas | Portland metro region (PPS + east-Multnomah + Washington/Clackamas-area districts), composite cells | geometry | `https://services.arcgis.com/quVN97tn06YNGj9s/arcgis/rest/services/School_Boundaries/FeatureServer` (layer 0; one cell layer, school per grade band in `Grade_1/6/10_Choice1_Name`, district in `Unified_SD_Name`) | VERIFIED (2026-06-11, live pull) | Found via the City's "Schools, School Attendance Areas, and School Districts" web map (item `e713b535...`); data last edited 2025-11. Replaces the retired COP_OpenData MapServer ("service not started") and the discontinued statewide `OREGON_EDUCATIONAL_BOUNDARIES` service. Dual-assignment cells ("Jefferson / Roosevelt") are split into both catchments. CLI: `ingest-boundaries cop_metro`. |
...
| PortlandMaps attendance | replaced by City of Portland AGOL `School_Boundaries` FeatureServer (see boundary table above) | RESOLVED 2026-06-11 | Portland metro attendance polygons ingest as `cop_metro` |
